...
- Never reintroduce cloud AI calls into the app — nothing about capture, OCR, or the AI ever leaves the device. Licensing is fully offline (Schicht 6): license keys are ECDSA-signed and verified on-device against an embedded public key, so activation makes no network call — the merchant of record delivers the signed key by email. The app's only UNPROMPTED network contact is the update check. Schicht 7 adds direct billing integrations (Moco/awork/Clio): these are the one user-initiated outbound transmission — strictly opt-in, and they send ONLY the confirmed entry the user pushes (date, mapped project, billed minutes, narrative), never captured activity, screenshots, OCR, evidence, or AI data. Same category as the file export: user-controlled data portability, not telemetry. Keep it that way.
